...
$.initialize(".Post .PostMeta time[data-humantime]", function () {
  let $te = $(this);
  ...
  const updater = () => {
    if (!document.contains($te[0])) {
      clearInterval(intervalKey);
      return;
    }

    $te.html(`${d.toLocaleDateString()} at ${d.toLocaleTimeString()}`);
  };

  updater();
  intervalKey = setInterval(updater, 10000);
});
